...
pub async fn discord_login() -> Result<String> {
    ...
    // 3) attendre le retour du navigateur sur 127.0.0.1:port (bloquant, timeout via thread)
    ...
    let html = "<html><body style='font-family:sans-serif;background:#0b0a12;color:#e9e6f2;text-align:center;padding-top:80px'>\
                <h2>Connecte. Tu peux fermer cet onglet et revenir dans PulseBoost.</h2></body></html>";
